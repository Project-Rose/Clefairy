...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!interaction.memberPermissions?.has('ManageMessages')) {
    return interaction.reply({ content: 'You do not have permission to manage messages, you are such a goof ball!', ephemeral: true });
  }

  if (amount < 1 || amount > 100) {
    return interaction.reply({ content: 'You need to enter a number between 1 and 100.', ephemeral: true });
  }
