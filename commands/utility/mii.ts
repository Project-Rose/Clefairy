...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption(option =>
    option.setName('render')
      .setDescription('Select the render you want to use (default: Wii U/Miiverse).')
      .setRequired(true)
      .addChoices(
  ...
  .addStringOption(option =>
    option.setName('expression')
      .setDescription('Select the expression you want to use (default: Normal).')
      .setRequired(true)
      .addChoices(
  ...
  .addStringOption(option =>
    option.setName('type')
      .setDescription('Select how your Mii will be positioned (default: Portrait).')
      .setRequired(true)
      .addChoices(
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!nid || (miipnid && miinnid)) {
    return interaction.reply({ content: "Please enter either a PNID or NNID, but not both.", ephemeral: true });
  }
  ...
  try {
    const [imageResponse, dataResponse] = await Promise.all([fetch(miiImageURL), fetch(miiDataURL)]);

    if (!imageResponse.ok) {
      return interaction.reply({ content: "I couldn't find your Mii, was it all just in your imagination perhaps?.", ephemeral: true });
    }
    ...
    const MiiQrAttachment = new AttachmentBuilder(miiQrStream, { name: 'mii_qr.png' });

    const embed = buildMiiEmbed(miiData, api_id);
    await interaction.reply({ embeds: [embed], files: [attachment, MiiQrAttachment] });

  } catch (error) {
    console.error('Error fetching Mii:', error);
    await interaction.reply({ content: "Couldn't fetch your sillyyyy Mii for some reason. Please try again.", ephemeral: true });
  }
}
...
function buildMiiEmbed(miiData: any, api_id: number) {
  const networkInfo = api_id === 1
    ...
    : { idType: 'NNID', color: '#FF7D00', text: 'Environment: Nintendo Network', favicon: 'https://media.discordapp.net/attachments/1290013025633304696/1295610730904817684/image.png?ex=670fefc5&is=670e9e45&hm=82d1fd7070057021150928c3410eed859c0feac65aabd30dbe6be6f064ff76ac&=&format=webp&quality=lossless' };
